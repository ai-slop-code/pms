...
// NukiGuestDailyEntry is the persisted first guest unlock per named stay and
// calendar day that powers the guest check-in heatmap.
...
// (property, named stay, day). The reconciler is the only writer; it always
...
func (s *Store) UpsertNukiGuestDailyEntry(ctx context.Context, row *NukiGuestDailyEntry) error {
	if !row.NamedStayID.Valid || row.NamedStayID.Int64 <= 0 {
		return errors.New("nuki guest daily entry requires named_stay_id")
	}
	...
	_, err := s.DB.ExecContext(ctx, `
		INSERT INTO nuki_guest_daily_entries (property_id, named_stay_id, day_date, first_entry_at, nuki_event_reference, created_at)
		VALUES (?, ?, ?, ?, ?, ?)
		ON CONFLICT(property_id, named_stay_id, day_date) WHERE named_stay_id IS NOT NULL DO UPDATE SET
			first_entry_at = excluded.first_entry_at,
			nuki_event_reference = excluded.nuki_event_reference`,
		row.PropertyID, row.NamedStayID.Int64, row.DayDate, first, ref, now)
	return err
}

// ListNukiGuestDailyEntriesInRange returns every (named stay, day) first
...
func (s *Store) ListNukiGuestDailyEntriesInRange(ctx context.Context, propertyID int64, fromDate, toDate string) ([]NukiGuestDailyEntry, error) {
	rows, err := s.DB.QueryContext(ctx, `
		SELECT g.id, g.property_id, g.named_stay_id, g.day_date, g.first_entry_at, g.nuki_event_reference, g.created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.PropertyID, &r.NamedStayID, &r.DayDate, &first, &r.NukiEventReference, &created); err != nil {
			return nil, err
		}
...
// to resolve a Smartlock event's authID back to the owning named stay.
//
...
func (s *Store) ListGeneratedNukiAccessCodesByExternalID(ctx context.Context, propertyID int64) (map[string]NukiAccessCodeIdentity, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ext, &ident.NamedStayID); err != nil {
			return nil, err
		}
		out[ext] = ident
	}
