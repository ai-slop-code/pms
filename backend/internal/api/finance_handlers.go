		...
		if err == nil && existing != nil {
			...
			if (!existing.TransactionID.Valid || existing.TransactionID.Int64 <= 0) && existing.NetCents != 0 {
				txInput := &store.FinanceTransaction{
					...
					Direction:       "incoming",
					AmountCents:     existing.NetCents,
					CategoryID:      sql.NullInt64{Int64: bookingIncomeCategoryID, Valid: true},
					Note:            sql.NullString{String: "Booking.com payout " + existing.ReferenceNumber, Valid: true},
					SourceType:      "booking_payout",
					SourceReference: sql.NullString{String: existing.ReferenceNumber, Valid: true},
					IsAutoGenerated: true,
				}
				if _, backfillErr := s.Store.BackfillBookingPayoutTransaction(r.Context(), existing.ID, txInput); backfillErr != nil {
					resp.Failed++
					continue
				}
				resp.Backfilled++
